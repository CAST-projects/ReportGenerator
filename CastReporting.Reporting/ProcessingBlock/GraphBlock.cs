...
namespace CastReporting.Reporting.Builder.BlockProcessing
{
    ...
    {
        #region PROPERTIES
        private static string BlockTypeName { get { return "GRAPH"; } }
        ...
        private static void ApplyContent(ReportData pClient, OpenXmlPackage pPackage, BlockItem pBlock, TableDefinition pContent, Dictionary<string, string> pOptions)
        {
            ...
            {
                string chartId = null;
                IEnumerable<XElement> allElementInPlaceHolder = null;

                var phElem = GetElements(pBlock.OxpBlock, pClient.ReportType);
                if (null == phElem)
                {
                ...
                allElementInPlaceHolder = GetElementsInPlaceHolder(pClient, phElem);
                XElement graphicElement = phElem.Descendants(A.graphic).FirstOrDefault();
                if (null == graphicElement)
                {
                    graphicElement = phElem.FirstOrDefault(_ => A.graphic.Equals(_.Name));
                }
                if (null != graphicElement)
                {
                        ...
                        if (null != embedPackage)
                        {
                            using (var ms = new MemoryStream())
                            {
                                ...
                                using (SpreadsheetDocument spreadsheetDoc = SpreadsheetDocument.Open(ms, true))
                                {
                                    ...
                                    if (null != shData)
                                    {
                                        IEnumerable<XElement> allRows = shData.Descendants(S.row);
                                        int ctRow = 0;
                                        int ctCell;
                                        ...
                                        for (int ctn = 0; ctn < nbRows; ctn += 1)
                                        {
                                            ...
                                            if (oneRow.Attribute(NoNamespace.r) != null)
                                                oneRow.Attribute(NoNamespace.r).Value = idxRow.ToString();
                                            idxRow += 1;
                                        }
                                        // ====================================================================================
                                        ...
                                        if (pContent.NbRows > nbRows)
                                        {
                                            ...
                                            if (oneSerieRow != null)
                                            {
                                                ...
                                                while (nbRows < pContent.NbRows)
                                                {
                                                    ...
                                                    newRow.Attribute(NoNamespace.r).Value = (nbRows + 1).ToString(); // DCO Correction, ROW ID starts at 1
                                                    ...
                                                    foreach (var oneCell in serieCells)
                                                    {
                                                        ...
                                                        oneCell.Attribute(NoNamespace.r).Value = String.Concat(WorksheetAccessor.GetColumnId(indexCol), newRowValue.ToString());
                                        ...
                                        if (sheetDimension != null && sheetDimension.Attribute(NoNamespace._ref) != null)
                                        ...
                                        int contentEltCount = null != pContent && null != pContent.Data ? pContent.Data.Count() : 0;
                                        // Apply values =======================================================================
                ...
                case FormatType.Word:
                    {
                        ...
                        {
                            return block.Select(_ => XElement.Parse(_.OuterXml));
                        }
                    }
        ...
        private static string GetSpreadsheetId(ChartPart chartPart)
        {
            string id = null;
            XElement extData = chartPart.GetXDocument().Descendants(C.externalData).FirstOrDefault();
            if (null != extData && null != extData.Attribute(R.id))
            {
                id = extData.Attribute(R.id).Value;
            }
            return id;
        }
        private static void UpdateCachedValues(ReportData conf, ChartPart chartPart, SpreadsheetDocument spreadsheetDoc, TableDefinition content)
        {
            try
            {
                    ...
                    for (int ctSer = 0; ctSer < cSerie.Count; ctSer += 1)
                    {
                        ...
                        foreach (XElement oneCache in cachedBlock)
                        {
                            ...
                            if (formula != null)
                            {
                                ...
                                if (sheet != null)
                                {
                                    ...
                                    if (null != ptCountCell && null != ptCountCell.Attribute(NoNamespace.val))
                        ...
                        if (content.GraphDataLabelText)
                        {
                            ...
                            if (tempLbl != null)
                            {
                                ...
                                if (content.DataLabel.Count() > nbDataLabels)
                                {
                                    ...
                                    while (content.DataLabel.Count() > nbDataLabels)
                                    {
                                        ...
                                        if (null != idxValue && null != idxValue.Attribute(NoNamespace.val))
                                        {
                            ...
                            for (int ctDlbl = 0; ctDlbl < nbDlbl; ctDlbl += 1)
                            {
                                ...
                                if (null != idxTag && null != idxTag.Attribute(NoNamespace.val))
                                {
                                ...
                                if (null == indexVal ||
                                    null == indexVal.Attribute(NoNamespace.val) ||
                                    "".Equals(indexVal.Attribute(NoNamespace.val).Value))
                                {
                                    continue;
                                }
                                ...
                                if (textTag != null)
                                {
                                    ...
                                    if (textValue != null) { textValue.SetValue(content.DataLabel.ElementAt(idxDataLbl)); }
                                }
                            }
                        }
